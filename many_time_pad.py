...
if __name__ == "__main__":
    ...
    xorNum1 = 7
    xorNum2 = 10
    xoredText = strxor(cypherTextsDecoded[xorNum1], cypherTextsDecoded[xorNum2])

    textLenth = len(xoredText)
    for i in range(textLenth):
        ...
        if(isreadable(cribDraggedText)):
            printCodePos(i, cribDraggedText, textLenth, ".")

    print("Guessed Key\n")
    key = strxor(cypherTextsDecoded[xorNum1], crib)
    print("key:%s" %key.encode("hex"))

    print("\nDecryption\n")
    for i, cypher in enumerate(cypherTextsDecoded):
        print("[%d] %s" % (i, strxor(cypher, key)))
